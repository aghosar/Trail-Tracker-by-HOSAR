...
export default function HomeScreen() {
  ...
  const sendSMS = async (phoneNumber: string, type: 'start' | 'update' | 'complete' | 'sos', lat: number, lon: number) => {
    try {
      ...
      if (!isAvailable) {
        console.log('HomeScreen: SMS not available - device does not support SMS or running on simulator/web');
        
        // Show user-friendly message only on real devices (not web/simulator)
        if (Platform.OS !== 'web') {
          showFeedback(
            'SMS Unavailable',
